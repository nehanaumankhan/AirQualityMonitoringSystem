...
const unsigned long otaUpdateInterval = 15 * 60 * 1000;
...
void update() {
  ...
  url += "k=77a1fe3f-f930-4cc3-901d-f1ee48bec89b";  // api key
  url += "&v=1.0.0.4";  // UPDATED version
  url += "&s=" + getChipId();

  WiFiClient client;
  httpUpdate.update(client, url, "1.0.0.4");
}
...
void setup() {
  ...
  Wifi_Init();
  configTime(5 * 3600, 0, "pool.ntp.org", "time.nist.gov");  //Line added to get current date time
  firebase_init();
  dht.begin();
...
void loop() {
  ...
  Serial.println(" minute(s)\nFirmware Version 1.0.0.4 successfully uploaded ALHAMDULILLAH!");
